...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    ...
    if (data === 'skip_comment') {
      const sess = getSession(ctx.from.id);
      if (sess.state === 'rating_comment') {
        await saveRating(ctx, sess.data, null);
      }
      return;
    }
...
bot.on('text', async (ctx) => {
  ...
  switch (sess.state) {
    ...
    // ── Rating Comment ──
    case 'rating_comment': {
      const comment = text.substring(0, 100);
      return saveRating(ctx, sess.data, comment);
    }
    ...
    case 'admin_whitelist_id': {
      ...
      clearSession(ctx.from.id);
      const target = userQueries.findByTelegramId.get(text.trim());
      if (!target) return ctx.reply('User not found. They must /start the bot first.');
...
async function finishPostAd(ctx, data, note) {
  ...
  await ctx.reply(
    `✅ *Ad #${adId} Posted Successfully!*\n\n${require('./helpers').formatAd({ ...ad, name: user.name, total_trades: user.total_trades, reputation: user.reputation })}` +
